...
@view.route('/store/<int:store_id>',  methods=['POST'])
...
@view.route('/store/<string:store>/drink/<int:drink_id>',  methods=['POST'])
def delete_drink(store, drink_id):
    Drink.query.filter_by(id=drink_id).delete()
    db.session.commit()
    current_app.logger.info("deleted store with id {}".format(drink_id))
    return redirect(url_for('view.dashboard_drinks', store=store))
